...
if [ -f ./build_src.tgz ]; then
	echo "#### FFmpeg static build, use local src ####"
	cp ./build_src.tgz $BUILD_DIR
	cd $BUILD_DIR
	tar zxf build_src.tgz
else
	...
	../fetchurl "http://qiniupk.qiniudn.com/ffmpeg-2.2.1.qiniu.tar.bz2"
	../fetchurl "http://nchc.dl.sourceforge.net/project/opencore-amr/opencore-amr/opencore-amr-0.1.3.tar.gz"
	../fetchurl "http://downloads.xiph.org/releases/speex/speex-1.2rc1.tar.gz"
	git clone https://github.com/dekkers/libilbc libilbc
	tar zcf $ENV_ROOT/build_src.tgz *
fi
...
echo "*** Building lame ***"
cd $BUILD_DIR/lame*
./configure --prefix=$TARGET_DIR --enable-static --disable-shared --enable-expopt
make -j $jval
make install
